...
export default function CMSMembersPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [bannerRes, categoriesRes, digitalCategoriesRes, partnerLogosRes, membershipBenefitsRes] = await Promise.all([
          ...
          <Card className="p-6">
            ...
            {expandedSections.membership && (
              <>
                ...
                <Button onClick={() => handleSave('membership')} disabled={isSaving === 'membership'} className="mt-3 h-8 text-xs">
                  {isSaving === 'membership' ? t({ en: 'Saving...', id: 'Menyimpan...' }) : t({ en: 'Save Membership Benefits', id: 'Simpan Manfaat Keanggotaan' })}
                </Button>
                </div>
              </>
            )}
